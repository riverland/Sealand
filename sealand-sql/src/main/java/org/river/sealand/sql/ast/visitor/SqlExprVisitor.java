 ...
 * @since Nov 25, 2013
 */
public class SqlExprVisitor extends AntlrTreeVisitor {

	@Override
	protected SqlCalcExpr doVisit(ParseTree tree, Parser parser) throws SQLException {

		ParseTree firstExpr = tree.getChild(0);
		...
		SqlCalcExpr calc = this.doVisit(firstExpr, parser);
		for (int i = 1; i < tree.getChildCount(); i = +2) {
			ParseTree node = tree.getChild(i);
			String operator = ((TerminalNode) node).getSymbol().getText();
			SqlCalcExpr tmp = this.doVisit(tree.getChild(i + 1), parser);
