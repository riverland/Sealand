 ...
 * @since Nov 30, 2013
 */
public class PendingTaskWatcher implements Watcher, Runnable {
	private static final Logger log = LoggerFactory.getLogger(PendingTaskWatcher.class);
	...
	@Override
	public void run() {
		final String taskListPath = NodeInfo.nodePath + "/" + TaskInfoPath.NODE_PENDING_TASK_LIST_PATH;
		while (true) {
			try {
				synchronized (atom) {
